...
import com.librarySpring.librarySpring.Entities.Person.services.GetPeopleService;
...
public class GetPeopleServiceTests {
    ...
    void shouldReturnAllPeople() {
        ...
        // When
        ResponseEntity<List<PersonDTO>> response = getPeopleService.execute(null);
        ...
        assertEquals("user1", personDTOs.get(0).getUsername());
        assertEquals("password1", personDTOs.get(0).getPassword());
        assertEquals("ADMIN", personDTOs.get(0).getRole());

        assertEquals("user2", personDTOs.get(1).getUsername());
    ...
    @Test
    void shouldReturnEmptyListWhenNoPeopleExist() {
        // Given
